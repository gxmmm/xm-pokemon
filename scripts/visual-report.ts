...
for (const scene of WORLD_SCENES) {
  ...
  console.log(`World scene ${budget.mapId}: static=${budget.staticContainerCount}/${scene.resources.staticContainerLimit}, entities=${budget.dynamicEntityCount}/${scene.resources.entityLimit}, ambientParticles=${budget.ambientParticleCount}/${scene.resources.ambientParticleLimit}, preload=${budget.preloadKeyCount}, baseline=${worldSceneFingerprintHash(scene)}, fingerprint=${budget.fingerprint}`);
}
