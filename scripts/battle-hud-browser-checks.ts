...
export async function checkBattleHud(page: Page, output: string): Promise<void> {
  await page.evaluate(() => window.__PLAYABLE_FIXTURE__.prepare('long', 1));
  ...
  assert.match(await cards.nth(0).locator('.mc-skill').nth(1).innerText(), /大字爆炎[\s\S]*4.2秒/);
  assert.match(await cards.nth(0).locator('.mc-skill').last().innerText(), /普通攻击/);
  ...
  for (const [width, height] of [[1366, 768], [1440, 900], [1920, 1080]]) {
    ...
    await page.screenshot({ path: resolve(output, `hud-${width}x${height}.png`) });
  }
  ...
  await page.screenshot({ path: resolve(output, 'hud-interrupt.png') });
