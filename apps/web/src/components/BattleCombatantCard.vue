...
<template>
  <article class="mon-card" :class="{ fainted: !combatant.alive, casting: !!hud.cast, critical: hud.critical }" :data-uid="combatant.uid" :aria-label="combatant.name">
    ...
    <div class="mc-skills">
      <span v-for="skill in hud.skills" :key="skill.id" class="mc-skill" :class="{ ready: skill.ready, active: skill.casting }" :style="{ '--skill-color': skill.color }" :title="skill.detail" tabindex="0" :aria-label="skill.detail">
...
.mc-skill:focus-visible { outline:1px solid #b9dfff; outline-offset:1px; }
