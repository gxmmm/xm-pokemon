...
const props = defineProps<{ instance: PokemonInstance; showHp?: boolean; selectable?: boolean; selected?: boolean; fainted?: boolean; compact?: boolean }>();
...
<template>
  <div class="pcard" :class="{ selected, fainted, compact }" :style="{ borderColor: selected ? 'var(--accent)' : undefined }">
    <div class="pcard-top">
      <PokemonSprite :species-id="instance.speciesId" :size="compact ? 40 : 56" :faded="fainted" />
      <div class="pcard-info">
        ...
        <div class="row" style="gap:4px;margin:2px 0;flex-wrap:wrap">
          ...
          <span v-if="!compact" class="chip sm-chip">{{ personality?.name }}</span>
        </div>
        <template v-if="!compact && showHp !== false">
        ...
        <div class="tiny muted" v-if="ability && !compact">{{ ability.name }}</div>
      </div>
    </div>
  </div>
</template>
...
.pcard.compact { padding: 5px 7px; border-radius: 10px; }
