 ...
 */
import type { BattleEvent, BattleVfx, TypeName } from '@pokemon-online/shared';
import { CanvasVfxPrimitives } from './canvas/CanvasVfxPrimitives.ts';
import { typeColor, STATUS_COLOR, type CanvasEffect, type FxFamily, type SkillFxProfile } from './canvas/types.ts';
export { getCanvasFxImage as getFxImage, loadCanvasFxAssets as loadFxAssets } from './canvas/assets.ts';
type Pt = { x: number; y: number };
/** Forward-lunge impulse applied to the attacker when it commits to an attack. */
...
interface Impact {
  x: number; y: number; targetUid: string; color: string; intensity: number;
  heavy: boolean; ko: boolean; secondary: boolean; delay: number;
}
...
export class EffectManager {
  ...
  private readonly primitives = new CanvasVfxPrimitives();
  ...
  private push(e: CanvasEffect): void {
  ...
  draw(ctx: CanvasRenderingContext2D): void {
    this.primitives.draw(ctx, this.effects);
  }
}