...
export function drawWaterShot(graphic: Graphics, x: number, y: number, nx: number, ny: number,
  ...
  const shoulder = point(-width * .7, -width * .55), flank = point(-length * .6, width * .5);
  graphic.poly([nose.x,nose.y,shoulder.x,shoulder.y,upper.x,upper.y,tail.x,tail.y,flank.x,flank.y,lower.x,lower.y])
    .fill({ color, alpha: 0.96 });
  const shine = point(-8, -width * 0.32), end = point(-length * 0.7, -width * 0.10);
  graphic.moveTo(shine.x, shine.y).lineTo(end.x, end.y).stroke({ color: spec.highlight, alpha: 0.9, width: 3 });
