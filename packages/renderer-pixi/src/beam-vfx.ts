...
export function spawnBeam(runtime: BattleEffectPool, from: BattleStagePoint, to: BattleStagePoint, color: number, intensity: number, variant = 'default', element?: TypeName, resolveSource?: () => BattleStagePoint | undefined): void {
  ...
  runtime.add(graphic, duration, (progress) => {
    ...
    if (!source) return;
    from = source;
    const alpha = Math.min(1, progress / BATTLE_SKILL_DETAILS.beam.ignitionRatio)
      * Math.min(1, (1 - progress) / BATTLE_SKILL_DETAILS.beam.fadeRatio) * 0.92;
    ...
    const px = -ny;
    const py = nx;
    const width = BATTLE_SKILL_DETAILS.beam.baseWidth + intensity * BATTLE_SKILL_DETAILS.beam.strengthWidth;
    graphic.clear();
    if (shape === 'flame') {
      ...
      for (let ring = 0; ring < 6; ring++) {
        ...
        graphic.moveTo(cx - nx * 5, cy - ny * 5).lineTo(cx + nx * 5, cy + ny * 5)
          .stroke({ color, alpha: alpha * 0.75, width: 2.4 });
      }
    } else {
      const energy = variant === 'meteor' ? BATTLE_SKILL_DETAILS.beam.energyColor : color;
      graphic.moveTo(from.x, from.y).lineTo(to.x, to.y).stroke({ color: energy, alpha: alpha * 0.35, width })
        .moveTo(from.x, from.y).lineTo(to.x, to.y).stroke({ color: energy, alpha, width: width * 0.42 });
      for (let i = 0; i < BATTLE_SKILL_DETAILS.beam.flowCount; i++) {
        const t = (i / BATTLE_SKILL_DETAILS.beam.flowCount + progress * 1.5) % 1;
        const start = Math.max(0, t - 0.075);
        graphic.moveTo(from.x + dx * start, from.y + dy * start).lineTo(from.x + dx * t, from.y + dy * t)
          .stroke({ color: 0xfff3cb, alpha: alpha * BATTLE_SKILL_DETAILS.beam.coreOpacity, width: 1.8 });
      }
    }
    if (variant === 'meteor') drawElementMotes(graphic, 'generic', to.x, to.y, progress, color, 4, 15, 12);
  });
}
