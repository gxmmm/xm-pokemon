...
export class BattleStage implements BattleRenderer {
  ...
  getDiagnostics(): BattleStageDiagnostics {
    ...
    return {
      ...
      effectChildCount: this.effectPool.childCount,
      totalChildCount: this.environmentView.childCount + this.combatants.container.children.length + this.effectPool.childCount + this.overlay.children.length,
  ...
  private update(dt: number): void {
    ...
    this.camera.update(dt, [
      ...
      { layer: this.effectPool.groundContainer, factor: 1, shake: true },
