...
- 范围爆发使用普通透明混合保留元素颜色，最大扩散半径、火焰高亮核心与精神光环数量由 `BATTLE_EFFECT_COMPOSITION` 统一约束，减少多种范围技能同时覆盖角色时的泛白。独立伤害命中反馈与特效时长保持不变；标准和减少闪烁模式分别采样 80 / 160 / 320 ms，检查三个目标完整反馈、睡眠标记及结束后释放。验收要求见 `doc/战斗表现第一轮验收.md`。
