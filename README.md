...
覆盖资源加载中退出、6 轮 3v3 进入/重开/退出、5 种环境连续切换、60 秒持续采样与 390px 窄屏布局，并采集喷射火焰、暗影球的分段动作截图。独立的确定性场景检查普通命中及致命一击的血量、存活状态和画面衔接，不接入正式页面或玩家存档。截图及 JSON 报告输出到 Git 忽略的 `doc/visual-baselines/battle/`。此项使用 SwiftShader 软件渲染，只作为功能与资源生命周期回归，不等于真实显卡性能、长时间压力测试或正式剧情流程验收。
...
- 经确认的引擎移动避让：选格同时检查从当前平滑坐标到目标格的整段路径，避开其他存活单位尚未走完的路径；冲突时复用原有绕行候选，否则等待。`battle-movement.ts` 的 `BATTLE_MOVEMENT.pathClearance` 统一声明半格移动安全间隔，已重叠的旧阵型只允许向外脱离，倒下立即释放阻挡。此规则会影响接敌路径与战局，但不修改技能射程、伤害、速度参数或站定时的格子间距，也不保证大幅角色贴图完全无遮挡。
