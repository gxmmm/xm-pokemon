...
- 默认开场为我方左下、敌方右上，固定全景为角色与 HUD 留出边缘空间；自定义阵型保留。终局撤销未释放蓄力，正常等待命中与倒下表现后自动结算。
- 战斗按角色作战距离持续进攻：近战预约接敌位置，远程保距压制；喷射、光束、爆发与周身范围按实际覆盖结算，像素蓄力范围与固定发射方向对应。
- 每种宝可梦固定一个首位主动招式，另有最多四个战术招式；首招可为攻击或友方治疗。速度显著缩短首招冷却、较温和地缩短其它冷却，同时保留完整动作与收招。
- 晴、雨、雪、沙暴通过顶部圆标展示并驱动条件特性；新天气覆盖旧天气，持续20秒。条件增益开启与结束时显示角色文字提示。
- 特性采用命中、属性减伤、目标增伤、受击削弱、冷却与临时属性等简单规则；AI 结合免疫、反制冷却与天气生命代价选敌选招。
- 玩家可见介绍与悬停提示使用中文，只说明实际规则；基础冷却、触发条件及目标范围与配置一致。
- 队伍协作：治疗结合有效缺血、承压与先到的友疗/自疗分工，濒危目标优先快速救援，前排就近反制后排身边的威胁，输出利用可赶上的控制窗口；支援者保留远程进攻手段。
- 范围避让：观察敌方公开蓄力，结合性格、血量和剩余时间进行最多两步的短移；遵守碰撞、动作占用与远程距离，以实际脚点判定是否躲开。
- 技能决策：按有效覆盖、当前移动与有效伤害选择招式；残血有快速收尾机会时减少长蓄力，控制结合实际打断窗口及队友正在施放的可靠控制；范围招式比较原地可覆盖的瞄准方向，收益接近时先用原地可释放招式，低压残局避免循环开盾。
