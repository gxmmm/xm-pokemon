...
角色动作与图标采用 [PMDCollab SpriteCollab](https://github.com/PMDCollab/SpriteCollab)；每只角色的原署名、固定源版本及转换记录随素材保留。旧 PokeAPI 素材作为历史资源保留。本项目为非商业同人作品，不用于商业用途。
...
- **全窗口布局**：外部页面、世界与战斗使用整个窗口，场景无套框或黑边；HUD 悬浮显示，画布随窗口调整实际尺寸，角色和地图保持等比。[布局验收说明](doc/桌面全窗口布局验收.md)。
...
- **战斗状态信息**：两侧卡片固定展示身份、生命、当前行动与技能；冷却保留技能名称，濒危、控制和施法可直接辨认。眩晕头顶标识与身体异常细节独立显示。[HUD 验收说明](doc/战斗HUD信息布局验收.md)。
...
| `npm run playable:browser` | 正式页面隔离验收：倍速、暂停跳过、结算次数、展开战报操作；1366/1440/1920 世界与战斗全窗口画布；1280×800 桌面页面布局与字号、技能说明定位、菜单和进化确认 |
...
- 151 只角色统一使用 PMDCollab 正反方向原生动作，`authoredFrames` 关闭旧的整图拉伸与额外前冲。逐帧源时长、脚点和头身挂点保存在序列帧元数据中；皮皮、皮可西使用 Dance 抬手施法并保留。缺少 Faint 的角色使用 Hurt 与统一淡出兜底。转换命令：`python scripts/import-pmd-battle-assets.py`（需 Pillow），说明见 [像素美术接入记录](doc/PMD像素美术接入与验收.md)。
- 全作采用同一像素标准：五种战斗环境使用固定调色板与像素构件，正式环境不再引用 AI 图片。投射物、命中、辅助、环境反馈使用阶梯轮廓；火焰使用像素采样和分级色块，水枪使用角状水块。统一输出网格覆盖世界与战斗，HUD 文字保持清晰；不提供画风或强度切换。
...
- 当前世界与战斗正式运行于 Pixi GPU，统一使用一套视觉与镜头标准；设置页不提供闪烁或镜头强度档位，旧浏览器视觉偏好不再读取。
- 六类技能样板沿用正式动作与命中链路：水枪为定向水体与局部水花，电击保留黄色电弧，光束快速点亮并保留流动亮纹、移除末端装饰环，空手劈为沿攻击来向的收尖短弧。火焰与暗影球继续使用专用效果。细节预算位于 `BATTLE_SKILL_DETAILS`；`npm run visuals:battle -- --skills-only` 采集标准正反两向的24个飞行／命中样本，见 `doc/六类技能样板验收.md`。
- 密集战斗中，脚步、环境反应与范围技能的地面碎片位于角色下方，发射与命中反馈保留前景；同一特效池负责两层的计时、配置透明度和释放。范围展开按唯一有效目标分发，地面反馈使用脚下挂点，群体展开亮度由统一配置约束，链状技能仍保持一条连续路径；不更改玩法站位与命中范围。
- 范围爆发使用普通透明混合保留元素颜色，最大扩散半径与高亮由 `BATTLE_EFFECT_COMPOSITION` 统一约束。通用装饰环改为水滴、短电弧、火舌或分散符片；蓄力使用向内聚集的短亮纹。异常状态采用像素细节：多处小火苗、紫烟、短电弧、局部冰晶，以及头顶 Z／问号／星标，参数和最小显示像素位于 `BATTLE_BODY_EFFECTS`。状态解除和倒下立即清空。
- 身体特效与脚部接触依据已加载纹理的可见边界适配体型；脚下阴影与窄草带共同校准，草带在角色自身层内排序，飞行、悬浮及倒下不生成草叶，遮挡预算位于 `BATTLE_FOOT_CONTACT`。这些调整不改变伤害、状态时长或移动规则。`npm run visuals:battle -- --natural-only` 可单独采集全部异常、元素、移动、净化、倒下及地形切换证据；完整战斗验收也包含这些检查，记录见 `doc/异常状态与脚部表现验收.md`。
...
战斗投影采用配置化的 50° 俯视角和统一 35° 水平斜角，同列角色在屏幕上错开；各环境镜头高度约束旋转后的场地范围。`BATTLE_BODY_DISPLAY` 限制大体型不透明身体的显示尺寸，小体型保留原尺寸；角色、脚点、阴影与技能挂点使用同一投影。引擎坐标、自由阵型、射程与间距规则不变。`npm run visuals:battle -- --spacing-only` 对照同一轨迹的开场、移动和停靠，在三种桌面尺寸下验证；允许自然局部交叠，不保证贴身战斗完全无遮挡。记录见 [大体型战斗可读性验收](doc/大体型战斗可读性验收.md)。

世界与战斗背景采用像素 2.5D 构件：房屋有屋檐厚度、侧墙与凹窗，码头有板面和支撑，塔层通过苔石、潮湿石材、晶簇、炉台和星辉柱区分。世界地面直接对应真实地图格，高物件按脚点与角色排序；战斗中央保留可读的交战平面。`npm run visuals:battle -- --relief-only` 采集六张世界地图与五种战斗背景的三档桌面截图，记录见 [像素场景体积改造](doc/像素场景体积改造.md)。

验证与构建命令统一经过 `scripts/run.mjs` 排队，同一项目只运行一个检查；Windows 默认低优先级、最多 2 个逻辑核心，子进程继承预算。完整战斗回归分为主体与异常/脚部两个批次，场景截图按三档分辨率拆批，每批关闭浏览器后休息 3 秒。检查内容不删减，耗时可能变长。`node --test scripts/verification-scheduler.test.mjs` 使用轻量进程验证排队、取消与异常退出后的恢复。完整战斗结果以本次生成的 `doc/visual-baselines/battle/suite-report.json` 为准，避免把旧批次报告当作新结果。
